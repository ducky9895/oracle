...
# from maddpg.trainer.trainer_OG import KLTrainer
# from maddpg.trainer.trainer_KL_full import KLTrainer
# from maddpg.trainer.trainer_MSE import KLTrainer
# from maddpg.trainer.trainer_KL_logp import KLTrainer
...
def get_trainers(env, num_adversaries, obs_shape_n, arglist):
    trainers = []
    model = mlp_model
    if arglist.mode == "infoEntro":
        print("information entropy mode")
        from maddpg.trainer.trainer_KL_infoEntro import KLTrainer
    elif arglist.mode == "annealing":
        print("annealing mode")
        from maddpg.trainer.trainer_OG import KLTrainer
    elif arglist.mode == "twoStage":
        print("Two stage")
        from maddpg.trainer.trainer_KL_2s import KLTrainer
    elif arglist.mode == "noKL":
        print("No KL")
        from maddpg.trainer.trainer_noKL_sw import KLTrainer
    else:
        print("Normal KL")
        ...
        while True:
            ...
            for i in range(env.n):
                if trainers[i].maddpg_mode:
                    action_n.append(trainers[i].train_action(obs_n[i])) if trainers[i].trainable else action_n.append(trainers[i].test_action(obs_n[i]))
                else:

...
class SoftCategoricalPd(Pd):
    ...
    def mse(self, other):
        ...
        p0 = ea0 / z0
        p1 = ea1 / z1
        return U.sum(tf.square(p0-p1), axis=1)
    def entropy(self):
        a0 = self.logits - U.max(self.logits, axis=1, keepdims=True)
        ea0 = tf.exp(a0)
        z0 = U.sum(ea0, axis=1, keepdims=True)
...
class MultiCategoricalPd(Pd):
    ...
    def nobias_sample(self):
        return self.low + tf.cast(tf.stack([p.nobias_sample() for p in self.categoricals], axis=-1), tf.int32)
    @classmethod
    def fromflat(cls, flat):
        return cls(flat)

class SoftMultiCategoricalPd(Pd):  # doesn't work yet #revised by jing
    def __init__(self, low, high, flat):
        self.flat = flat
        ...
        self.action_size = [categorical.sample().get_shape().as_list()[-1] for categorical in self.categoricals]
    def flatparam(self):
        return self.flat
    def mode(self):
        x = []
    ...
    def logp(self, x):
        return tf.add_n([p.logp(px - self.low[i]) for i, (p, px) in enumerate(zip(self.categoricals, tf.split(x, self.action_size, axis=len(x.get_shape()) - 1)))])
        # return tf.add_n([p.logp(px) for p, px in zip(self.categoricals, tf.unstack(x - self.low, axis=len(x.get_shape()) - 1))])
    def kl(self, other):
        return tf.add_n([
                p.kl(q) for p, q in zip(self.categoricals, other.categoricals)
...
class BernoulliPd(Pd):
    ...
    def nobias_sample(self):
        p = tf.sigmoid(self.logits)
        return tf.to_float(math_ops.less(0.5, p))
    @classmethod
    def fromflat(cls, flat):
        return cls(flat)
...
#     elif isinstance(ac_space, spaces.Discrete):
#         # return CategoricalPdType(ac_space.n)
#         print("Discrete Space")
#         return SoftCategoricalPdType(ac_space.n)
#     elif isinstance(ac_space, MultiDiscrete):
#         #return MultiCategoricalPdType(ac_space.low, ac_space.high)
#         print("MultiDiscrete Space")
#         for space in ac_space:
#             print(isinstance(space, spaces.Box))
#         return SoftMultiCategoricalPdType(ac_space.low, ac_space.high)
#     elif isinstance(ac_space, spaces.MultiBinary):
#         print("MultiBinary Space")
#         return BernoulliPdType(ac_space.n)
#     else:
#         raise NotImplementedError

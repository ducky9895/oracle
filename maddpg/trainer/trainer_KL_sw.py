...
"""Actor (Policy) Model."""
def p_train(make_obs_ph_n, act_space_n, p_index, full_p_func, q_func, local_p_func, optimizer, grad_norm_clipping=None, num_units=64, scope="trainer", reuse=tf.AUTO_REUSE):
    with tf.variable_scope(scope, reuse=reuse):
        ...
        local_p = local_p_func(local_p_input, int(act_pdtype_n[p_index].param_shape()[0]), scope=scope+"/local_p_func", num_units=num_units)
        full_p = full_p_func(full_p_input, int(act_pdtype_n[p_index].param_shape()[0]), scope=scope+"/full_p_func", num_units=num_units)
        
        full_p_func_vars = U.scope_vars(U.absolute_scope_name(scope+"/full_p_func"))
        local_p_func_vars = U.scope_vars(U.absolute_scope_name(scope+"/local_p_func"))
        
        # full loss
        full_act_pd = act_pdtype_n[p_index].pdfromflat(full_p)
        full_train_act_sample = full_act_pd.sample()
        full_test_act_sample = full_act_pd.nonbias_sample()
        full_p_reg = tf.reduce_mean(tf.square(full_act_pd.flatparam()))
        
        act_input_n[p_index] = full_train_act_sample
        q_input = tf.concat(obs_ph_n + act_input_n, 1)
        q = q_func(q_input, 1, scope=scope+"/q_func", reuse=True, num_units=num_units)[:, 0]
        ...
        q_input = tf.concat(obs_ph_n + act_input_n, 1)
        q_ = q_func(q_input, 1, scope=scope+"/q_func", reuse=True, num_units=num_units)[:, 0]
        local_pg_loss = -tf.reduce_mean(q_)
        
        # target network
        target_p = full_p_func(full_p_input, int(act_pdtype_n[p_index].param_shape()[0]), scope=scope+"/target_p_func", num_units=num_units)
        target_p_func_vars = U.scope_vars(U.absolute_scope_name(scope+"/target_p_func"))
        ...
        # KL divergence between the target p and local p
        ...
        p_values = [full_p, target_p, local_p, KL_loss]
        ...
        summary = [
            ...
            tf.summary.scalar('full_p_reg', full_p_reg),
            tf.summary.scalar('local_loss', local_loss),
            tf.summary.scalar('KL_loss', KL_loss),
            tf.summary.scalar('local_pg_loss', local_pg_loss),
            tf.summary.scalar('local_p_reg', local_p_reg)
        ]
...
def q_train(make_obs_ph_n, act_space_n, q_index, q_func, optimizer, grad_norm_clipping=None, local_q_func=False, scope="trainer", reuse=tf.AUTO_REUSE, num_units=64):
    with tf.variable_scope(scope, reuse=reuse):
        ...
        # create summary
        summary = [tf.summary.scalar('q_loss', loss)]
...
class KLTrainer(AgentTrainer):
    ...
    def full_train_action(self, obs):
        new_obs = [obs_.reshape(1, -1) for obs_ in obs]
        return self.full_train_act(*new_obs)[0]

    def full_test_action(self, obs):
        new_obs = [obs_.reshape(1, -1) for obs_ in obs]
        return self.full_test_act(*new_obs)[0]
    ...
    def update(self, agents, t, sw):
        ...
        for j in range(num_sample):
            target_act_next_n = [agents[i].p_debug['target_act'](obs_next_n[i]) if agents[i].maddpg_mode else agents[i].p_debug['target_act'](*obs_next_n) for i in range(self.n)]
        ...
        if t % self.args.save_rate == 0:
            ...
            for summary in p_loss_summary:
                tf.summary.scalar('p_loss_summary', summary, step=t)
            q_loss_summary = self.q_summary(*(obs_n + act_n + [target_q]))
            for summary in q_loss_summary:
                tf.summary.scalar('q_loss_summary', summary, step=t)
